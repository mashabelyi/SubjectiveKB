...
python3 eval_sourceModel.py --data [path to data] --name [path to pretrained model folder]
...
from transE import TransE_SourceFull
...
# INITIALIZE MODEL
# =========================================
transE = TransE_SourceFull(len(all_rels), len(ent2id), len(src2id), dim=config['embedding_dim'], norm=config['norm'])
transE.to(device)
